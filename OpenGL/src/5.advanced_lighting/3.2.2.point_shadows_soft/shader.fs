...
uniform samplerCube texture_depth;
...
//    float closestDepth = texture(texture_depth, fs_in.FragPos).r;
...
float ShadowCalculation(vec3 fragPos)
{
    ...
    float offset = range * 2 / (samples - 1.0);
    
//    for (float x = -range; x <= range; x += offset) {
//        for (float y = -range; y <= range; y += offset) {
//            for (float z = -range; z <= range; z += offset) {
//                float closestDepth = texture(texture_depth, fragToLight + vec3(x, y, z) * diskRadius).r;
//
...
//    shadow /= (samples * samples * samples );
    ...
    for (int i = 0; i < samples; ++i) {
        float closestDepth = texture(texture_depth, fragToLight + sampleOffsetDirections[i] * diskRadius).r;
        closestDepth *= farPlane;
        float currentDepth = length(fragToLight);
